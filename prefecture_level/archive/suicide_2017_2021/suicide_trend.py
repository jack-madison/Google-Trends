...
for x in prefectures:
    try:
        ...
        pytrends.build_payload(kw, timeframe='2017-01-01 2022-01-01', geo = str(region) + '-' + str(prefectures[x]))
        # Request the interest over time data for the time interval specified above
        weekly_2017_2021 = pytrends.interest_over_time()

        # Drop the unneeded column and rename the trend data column
        weekly_2017_2021 = weekly_2017_2021.drop('isPartial', 1)
        weekly_2017_2021.columns.values[0] = "raw_weekly"
        ...
        daily_2017_2021 = daily_jan_jun_2017.append([daily_jul_dec_2017, daily_jan_jun_2018, daily_jul_dec_2018, daily_jan_jun_2019, daily_jul_dec_2019, daily_jan_jun_2020, daily_jul_dec_2020, daily_jan_jun_2021, daily_jul_dec_2021])

        # Drop the unneeded column and rename the trend data column
        daily_2017_2021 = daily_2017_2021.drop('isPartial', 1)
        daily_2017_2021.columns.values[0] = "raw_daily"

        # Merge the weekly and daily trends data
        trends_data = pd.merge(daily_2017_2021, weekly_2017_2021, how = 'left', left_index = True, right_index = True)
...
# Export the data
trends_data_prefecture.to_csv('./prefecture_level/suicide/suicide_trends_data.csv', index = False)
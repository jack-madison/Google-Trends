...
for x in prefectures:
    try:
        ...
        pytrends.build_payload(kw, timeframe='2014-12-28 2020-01-04', geo = str(region) + '-' + str(prefectures[x]))
        # Request the interest over time data for the time interval specified above
        weekly_2015_2019 = pytrends.interest_over_time()

        # Drop the unneeded column and rename the trend data column
        weekly_2015_2019 = weekly_2015_2019.drop('isPartial', 1)
        weekly_2015_2019.columns.values[0] = "raw_weekly"
        ...
        daily_2015_2019 = daily_jan_jun_2015.append([daily_jul_dec_2015, daily_jan_jun_2016, daily_jul_dec_2016, daily_jan_jun_2017, daily_jul_dec_2017, daily_jan_jun_2018, daily_jul_dec_2018, daily_jan_jun_2019, daily_jul_dec_2019])

        # Drop the unneeded column and rename the trend data column
        daily_2015_2019 = daily_2015_2019.drop('isPartial', 1)
        daily_2015_2019.columns.values[0] = "raw_daily"

        # Merge the weekly and daily trends data
        trends_data = pd.merge(daily_2015_2019, weekly_2015_2019, how = 'left', left_index = True, right_index = True)
...
# Export the data
trends_data_prefecture.to_csv('./prefecture_level/suicide_2015_2019/suicide_trends_data.csv', index = False)
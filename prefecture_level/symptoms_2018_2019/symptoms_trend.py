...
for x in prefectures:
    try:
        ...
        # Build the payload for the weekly data between January 2018 and December 2019
        pytrends.build_payload(kw, timeframe='2017-12-31 2020-01-04', geo = str(region) + '-' + str(prefectures[x]))
        # Request the interest over time data for the time interval specified above
        weekly_2018_2019 = pytrends.interest_over_time()

        # Drop the unneeded column and rename the trend data column
        weekly_2018_2019 = weekly_2018_2019.drop('isPartial', 1)
        weekly_2018_2019.columns.values[0] = "raw_weekly"
        ...
        daily_2018_2019 = daily_jan_jun_2018.append([daily_jul_dec_2018, daily_jan_jun_2019, daily_jul_dec_2019])

        # Drop the unneeded column and rename the trend data column
        daily_2018_2019 = daily_2018_2019.drop('isPartial', 1)
        daily_2018_2019.columns.values[0] = "raw_daily"

        # Merge the weekly and daily trends data
        trends_data = pd.merge(daily_2018_2019, weekly_2018_2019, how = 'left', left_index = True, right_index = True)
...
# Export the data
trends_data_prefecture.to_csv('./prefecture_level/symptoms/symptoms_trends_data.csv', index = False)